...
import com.satya.musicplayer.extensions.addRemainingMediaItems
...
internal fun PlaybackService.getMediaSessionCallback() = object : MediaLibrarySession.Callback {
    ...
    var lastPlayPauseButtonClickedAt: Instant? = null
    ...
    override fun onMediaButtonEvent(
        ...
        when (keyEvent?.keyCode) {
            KEYCODE_MEDIA_PLAY_PAUSE -> {
                togglePlayer()
                if (lastPlayPauseButtonClickedAt == null) {
                    lastPlayPauseButtonClickedAt = Instant.now()
                } else if (Duration.between(
                        lastPlayPauseButtonClickedAt,
                        Instant.now()
                    ).seconds <= 2
                ) {
                    rewind()
    ...
    override fun onCustomCommand(
        ...
        when (command) {
            ...
            CustomCommands.PLAY_COMMAND -> {
                try {
                    args.getString("id")?.toIntOrNull()?.let {
                        playSpecificCommand(it)
                    }
                } catch (_: Exception) {}
            }
            CustomCommands.SKIP_COMMAND -> {
                skipCurrentCommand()
            }
        }
